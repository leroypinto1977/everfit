...
import { flushPurchase } from "@/lib/analytics";

function SuccessContent() {
  const params = useSearchParams();
  const orderId = params.get("order");

  // Fires the GA4 purchase event stashed by checkout; a no-op on reloads,
  // hand-typed URLs and email-link revisits, so conversions count once.
  useEffect(() => {
    if (orderId) flushPurchase(orderId);
  }, [orderId]);
