...
import { db } from "@everfit/core/db";
import { adminUsers } from "@everfit/core/db/schema";
import {
  createPasswordReset,
  createSession,
  isValidSetupKey,
...
import { sendPasswordResetEmail } from "@everfit/core/lib/notify";
import { adminUrl } from "@everfit/core/lib/email/render";
...
export async function requestPasswordReset(_prev: ResetState, formData: FormData): Promise<ResetState> {
  ...
  if (reset) {
    await sendPasswordResetEmail({
      ...
      resetUrl: adminUrl(`/reset?token=${reset.token}`),
    });
  }
