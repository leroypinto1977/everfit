...
import { getCustomer } from "@everfit/core/lib/customers";
import StatusBadge from "@/components/StatusBadge";
import type { OrderStatus } from "@everfit/core/lib/orders";
...
export default async function CustomerDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/customers" className="text-sm text-[#6b7194] hover:text-[#2b337d]">
          ← All customers
        </Link>
            ...
            {customer.orders.map((o) => (
              <tr key={o.id} className="border-t border-[#eef0f7] transition-colors hover:bg-[#f8f9fd]">
                <td className="px-6 py-4">
                  <Link href={`/orders/${o.id}`} className="font-mono text-xs text-[#2b337d] hover:underline">
                    {o.id}
                  </Link>
