...
/**
 * Absolute URL into the admin panel, which is a separate deployment on its own
 * (private) domain — see ADMIN_URL. Used by the owner-facing emails only.
 * Falls back to the storefront so a missing env var yields a reachable link
 * rather than a broken one.
 */
export function adminUrl(path = "") {
  const base = (process.env.ADMIN_URL ?? process.env.NEXT_PUBLIC_SITE_URL ?? "https://evherfit.com").replace(
    /\/$/,
    ""
  );
  return `${base}${path}`;
